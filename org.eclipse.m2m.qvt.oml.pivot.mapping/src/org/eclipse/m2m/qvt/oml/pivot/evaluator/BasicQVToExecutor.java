...
public class BasicQVToExecutor extends AbstractExecutor implements QVTiExecutor {
	...
	private static org.eclipse.qvtd.qvto.pivot.qvtimperative.evaluation.QVTiTransformationAnalysis createTransformationAnalysis(
			@NonNull QVTiEnvironmentFactory environmentFactory, @NonNull OperationalTransformation transformation) {
		org.eclipse.qvtd.qvto.pivot.qvtimperative.evaluation.QVTiTransformationAnalysis transformationAnalysis = environmentFactory
	...
	public static @NonNull OperationalTransformation loadTransformation(@NonNull Class<? extends Model> modelClass,
			@NonNull EnvironmentFactory environmentFactory, @NonNull URI transformationURI, boolean keepDebug)
					throws IOException {
		// Load the transformation resource
		CSResource xtextResource = (CSResource) environmentFactory.getResourceSet().getResource(transformationURI,
				true);
		if (xtextResource == null) {
			throw new IOException("Failed to load '" + transformationURI + "'");
		}
		String csMessage = PivotUtil.formatResourceDiagnostics(ClassUtil.nonNullEMF(xtextResource.getErrors()),
				"Failed to load '" + transformationURI + "'", "\n");
		if (csMessage != null) {
			throw new IOException(csMessage);
		}
		try {
			ASResource asResource = xtextResource.getASResource();
			String asMessage = PivotUtil.formatResourceDiagnostics(ClassUtil.nonNullEMF(asResource.getErrors()),
					"Failed to load '" + asResource.getURI() + "'", "\n");
			if (asMessage != null) {
	...
	// public BasicQVToExecutor(@NonNull QVTiEnvironmentFactory
	// environmentFactory, @NonNull URI transformationURI) throws IOException {
	// this(environmentFactory, loadTransformation(ImperativeModel.class,
	// environmentFactory, transformationURI, environmentFactory.keepDebug()));
	// }

	public BasicQVToExecutor(@NonNull QVTiEnvironmentFactory environmentFactory,
			@NonNull OperationalTransformation transformation) {
	...
	private BasicQVToExecutor(@NonNull QVTiEnvironmentFactory environmentFactory,
			@NonNull OperationalTransformation transformation,
			org.eclipse.qvtd.qvto.pivot.qvtimperative.evaluation.QVTiTransformationAnalysis qvTiTransformationAnalysis) {
	...
	public void execute() {
		...
		// QVTbaseUtil.getContextVariable(standardLibrary, transformation);
		//
		// QVTiModelManager modelManager = getModelManager();
		// add(ownedContext,
		// modelManager.getTransformationInstance(transformation));
		// TODO Auto-generated method stub
